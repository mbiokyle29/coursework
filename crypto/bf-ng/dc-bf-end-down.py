...
def main():
    ...
    start = 100000000000
    end   = 1000000000000 # global scope so we can report
    ...
    log.info("Starting brute force at %i", end)
    try:
        while(end != start):
            if check_dc_brute(end,g,p) == y:
                log.info("Discrete Log found, x = {}".format(end))
                return
            end -= 1

    except KeyboardInterrupt:
        log.info("Stopped at: {}".format(end))
