...
def main():
    start = mpz(100000000000)
    middle = mpz(150000000000)
    ...
    log.info("Starting brute force at %i", middle)
    try:
        while(middle != start):
            if check_dc_brute(middle,g,p) == y:
                log.info("Discrete Log found, x = {}".format(middle))
                return
            middle -= 1

    except KeyboardInterrupt:
        log.info("Stopped at: {}".format(middle))
