...
public class Maze {
	 ...
	 * @param j
	 * 
	 * @return boolean, if the given x,y pos is a valid matrix position
	 */
	public boolean checkSquareInBounds(int i, int j) {
		boolean valid = true;
		// If i or j are less then 0 then return false
		if(i < 0 || j < 0 ) {
			valid = false;
		// If i or j are too large then false
		} else if(i >= this.noOfRows || j >= this.noOfCols ) {
			valid = false;
		}
		
		// If neither if statement triggered, the i,j is valid
		return valid;
	}
}
